    ...
    def process(self, req):
        ...
        while (rospy.Time.now().secs - start.secs) < 3.5:
            ...
            # 輪郭に外接する長方形を取得する。
                ...
                result_pub = cv2.cvtColor(result, cv2.COLOR_RGB2BGR)
                result_pub = self.bridge.cv2_to_imgmsg(result_pub, "bgr8")
                self.detection_result_pub.publish(result_pub)
                #print(type(result))
                image_pil=PIL.Image.fromarray(result)
                ...
                with torch.no_grad():
                    ...
                    logits_per_image,logits_per_text=self.model(self.image, self.text)
